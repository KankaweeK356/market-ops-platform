...
const SEED_TEMPLATES = [
  // 1. จัดพื้นที่ - แผงเช่า
  {
    ...
    questions: [
      { id: "totalStalls", label: "จำนวนแผงค้าทั้งหมดในโซนที่ตรวจ (แผง)", type: "number", required: true },
      { id: "occupiedStalls", label: "จำนวนแผงที่มีการทำสัญญาเช่า/เปิดขายจริงวันนี้ (แผง)", type: "number", required: true }
    ]
  },
...
function generateSeedSubmissions() {
  ...
  return [
    // 1. ฝ่ายจัดพื้นที่ & สารพิษ
    {
      ...
      zone: ZONES[0],
      answers: [
        { questionId: "totalStalls", label: "จำนวนแผงค้าทั้งหมดในโซนที่ตรวจ (แผง)", value: 500 },
        { questionId: "occupiedStalls", label: "จำนวนแผงที่มีการทำสัญญาเช่า/เปิดขายจริงวันนี้ (แผง)", value: 480 }
      ],
      derivedStatus: "ปกติ",
      createdAt: now - 1 * day
    },
...
export function computeStats(submissions) {
  ...
  if (spaceOccupancySubs.length > 0) {
    let totalStalls = 0;
    let occupiedStalls = 0;
    spaceOccupancySubs.forEach(s => {
      const tot = findAnswer(s, "totalStalls") || 500;
      const occ = findAnswer(s, "occupiedStalls") || 480;
      totalStalls += tot;
      occupiedStalls += occ;
    });
    dynamicKPIs["d-space"].occupancy = totalStalls > 0 ? Number(((occupiedStalls / totalStalls) * 100).toFixed(1)) : 96.0;
  }
