...
const SEED_TEMPLATES = [
  ...
  // 2. จัดพื้นที่ - ตรวจสารพิษ (สุ่มตรวจสารพิษรายแผง)
  {
    id: "tmpl-space-toxin",
    activityId: "act-space-toxin",
    questions: [
      { id: "productType", label: "ประเภทผลผลิตที่สุ่มตรวจ", type: "single_choice", options: ["ผักใบ (Leafy Greens)", "ผลไม้ (Fruits)", "ผักหัว/เครื่องปรุง (Roots/Spices)"], required: true },
      { id: "stallCode", label: "รหัสแผงค้าที่สุ่มตรวจ (เช่น A-01, C-15)", type: "text", required: true },
      { id: "testResult", label: "ผลการตรวจหาสารเคมีปนเปื้อน", type: "single_choice", options: ["ปลอดภัย (Pass)", "พบสารเคมีปนเปื้อน (Not Pass)"], required: true, flagIf: { value: "พบสารเคมีปนเปื้อน (Not Pass)", setStatus: "ต้องติดตาม" } }
    ]
  },
  // 3. จัดพื้นที่ - ค่าปรับ (บันทึกยอดสั่งปรับรายธุรกรรม)
  {
    id: "tmpl-space-fines",
    activityId: "act-space-fines",
    questions: [
      { id: "violationType", label: "ประเภทความผิดที่ตรวจพบ", type: "single_choice", options: ["วางของกีดขวางทางเดิน (Stall Obstruction)", "ทิ้งขยะไม่เป็นที่ (Illegal Dumping)", "จอดรถกีดขวาง (Traffic Obstruction)", "อื่นๆ"], required: true },
      { id: "fineAmount", label: "มูลค่าสั่งปรับ (บาท)", type: "number", required: true, flagIf: { value: ">1000", setStatus: "ต้องติดตาม" } },
      { id: "violatorName", label: "ผู้กระทำผิด / รหัสแผงผู้ค้า", type: "text", required: true }
    ]
  },
...
function generateSeedSubmissions() {
  ...
  return [
    ...
    },
    {
      ...
      zone: ZONES[0],
      answers: [
        { questionId: "productType", label: "ประเภทผลผลิตที่สุ่มตรวจ", value: "ผักใบ (Leafy Greens)" },
        { questionId: "stallCode", label: "รหัสแผงค้าที่สุ่มตรวจ (เช่น A-01, C-15)", value: "C-15" },
        { questionId: "testResult", label: "ผลการตรวจหาสารเคมีปนเปื้อน", value: "พบสารเคมีปนเปื้อน (Not Pass)" }
      ],
      derivedStatus: "ต้องติดตาม",
      createdAt: now - 1 * day
    },
    {
      ...
      zone: ZONES[1],
      answers: [
        { questionId: "violationType", label: "ประเภทความผิดที่ตรวจพบ", value: "วางของกีดขวางทางเดิน (Stall Obstruction)" },
        { questionId: "fineAmount", label: "มูลค่าสั่งปรับ (บาท)", value: 12500 },
        { questionId: "violatorName", label: "ผู้กระทำผิด / รหัสแผงผู้ค้า", value: "แผงป้าศรี (Zone A-08)" }
      ],
      derivedStatus: "ต้องติดตาม",
      createdAt: now - 2 * day
    },
    ...
    },
    {
      ...
      zone: ZONES[0],
      answers: [
        { questionId: "routingRound", label: "ช่วงเวลา / รอบการเดินรถ", value: "รอบที่ 2 (11.00 - 15.00 น.)" },
        { questionId: "truckRoutingMinutes", label: "ระยะเวลาที่ใช้จัดรถเข้าอาคารผักเฉลี่ย (นาที)", value: 42 }
      ],
      derivedStatus: "ต้องติดตาม",
      createdAt: now - 1 * day
    },
...
export function computeStats(submissions) {
  ...
  if (spaceToxinSubs.length > 0) {
    ...
    spaceToxinSubs.forEach(s => {
      const result = s.resolved ? "ปลอดภัย (Pass)" : findAnswerStr(s, "testResult");
      baseTotal++;
      if (result === "ปลอดภัย (Pass)") {
        basePassed++;
      }
    });
    dynamicKPIs["d-space"].safety = Number(((basePassed / baseTotal) * 100).toFixed(1));
  }
  ...
  if (spaceRoutingSubs.length > 0) {
    ...
    spaceRoutingSubs.forEach(s => {
      const mins = s.resolved ? 28 : (findAnswer(s, "truckRoutingMinutes") || 33);
      totalMins += mins;
    });
  ...
  if (secRoutingSubs.length > 0) {
    ...
    secRoutingSubs.forEach(s => {
      const mins = s.resolved ? 33 : (findAnswer(s, "truckRoutingMinutes") || 42);
      totalMins += mins;
    });
...
export function resolveIncident(submissionId, decisionText) {
  ...
  if (idx !== -1) {
    ...
    if (data.submissions[idx].formTemplateId === "tmpl-space-toxin") {
      const ansIdx = answers.findIndex(a => a.questionId === "testResult");
      if (ansIdx !== -1) answers[ansIdx].value = "ปลอดภัย (Pass)";
    } else if (data.submissions[idx].formTemplateId === "tmpl-sec-routing") {
      const ansIdx = answers.findIndex(a => a.questionId === "truckRoutingMinutes");
      if (ansIdx !== -1) answers[ansIdx].value = 33;
    } else if (data.submissions[idx].formTemplateId === "tmpl-space-fines") {
      const ansIdx = answers.findIndex(a => a.questionId === "fineAmount");
