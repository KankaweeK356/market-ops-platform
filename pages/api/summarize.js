...
export default async function handler(req, res) {
  ...
  if (!geminiKey && !anthropicKey) {
    if (departmentId === "d-space") {
      return res.status(200).json({
        summary: "สรุปภาพรวมฝ่ายจัดพื้นที่ & สารพิษ: อัตราเช่าแผงค้ารวมทุกโซน (Stall Occupancy) ในสัปดาห์นี้ทำผลงานได้คงตัวที่ 96.0% (ผ่านเกณฑ์เป้าหมาย >=90%) อย่างไรก็ตาม ดัชนีสัดส่วนตรวจสารเคมีแล้วปลอดภัยในพืชผักตกลงมาอยู่ที่ 95.0% (เป้าหมาย 100%) เนื่องจากสุ่มคัดกรอง 65 รายการแล้วพบสารปนเปื้อนยาฆ่าแมลงตกค้างระดับเตือนในผักใบสะสม 5% และเวลาจัดจอดรถเทียบอาคารรถผักใน 3 รอบหลักเฉลี่ยอยู่ที่ 33 นาทีต่อรอบ ซึ่งหลุดเกณฑ์ที่ 30 นาที",
        anomalies: [
          "สุ่มตรวจพบยาฆ่าแมลงตกค้างในกลุ่มผักใบและคะน้าจีนคิดเป็นสัดส่วน 5% ของชุดสุ่ม",
          "เวลารอบจัดระเบียบเดินรถชานชาลาอาคารผักผลไม้ล่าช้าเฉลี่ยแตะ 33 นาทีต่อรอบ"
        ],
        recommendations: [
          "สั่งตักเตือนแผงค้าและงดจำหน่ายพืชผักปนเปื้อนชั่วคราว 3 วันตามเกณฑ์ปลอดภัย",
          "จัดสรรเจ้าหน้าที่ระบายจัดจราจรร่วมคิวรอบรถชานชาลาอาคารรถผักใหม่ในช่วงเวลาขนส่งสูงสุด"
        ]
      });
    } else if (departmentId === "d-specsec") {
      return res.status(200).json({
        summary: "สรุปภาพรวมฝ่าย รปภ. เฉพาะกิจ: ผลการปฏิบัติงานรักษาความสงบสัปดาห์นี้พบปัญหาเกิดเหตุทะเลาะวิวาทและลักทรัพย์บริเวณลานค้ารวม 2 ครั้ง (หลุดเกณฑ์เป้าหมายสะสมต้องเป็น 0) และการตรวจปัสสาวะสุ่มหาสารเสพติดประจำสัปดาห์ในกลุ่มแรงงานบัตรเสื้อม่วงจำนวน 320 ราย พบผลตรวจเป็นบวกมีพฤติกรรมเสพยา 2 ราย ซึ่งละเมิดต่อข้อตกลงสอดคล้องความมั่นคงตลาดอย่างร้ายแรง",
        anomalies: [
          "พบสถิติคดีทะเลาะวิวาท 2 ครั้งในตลาด ขัดเกณฑ์ KPI ความปลอดภัยที่เป็นศูนย์",
          "ตรวจปัสสาวะพบสารเสพติดในแรงงาน 2 รายระหว่างการสุ่มตรวจสอบรายกะ"
        ],
        recommendations: [
          "สั่งยกเลิกสิทธิ์เสื้อม่วงและใบอนุญาตแรงงาน 2 รายส่งดำเนินคดี พร้อมขยายผลตรวจสอบแผงต้นสังกัด",
          "อนุมัติเพิ่มกำลังเจ้าหน้าที่ตั้งจุดสกัดและเดินเท้าตรวจความมั่นคงรอบลานจอดรถในชั่วโมงหนาแน่น"
        ]
      });
    } else if (departmentId === "d-cold") {
      return res.status(200).json({
        summary: "สรุปภาพรวมฝ่ายห้องเย็น: อัตราใช้ประโยชน์และคะแนนพึงพอใจการใช้พื้นที่ฝากสินค้าแช่เย็นรายชิ้นทำได้ดีที่ 4.8 / 5.0 คะแนน และสัดส่วนบรรจุของอยู่ที่ 85% (ผ่านเกณฑ์มาตรฐาน) แต่ดัชนีค่าน้ำมันและกระแสไฟฟ้าห้องเย็นหลักพบการสิ้นเปลืองเกินเป้าหมาย โดยเฉพาะห้องแช่แข็ง A-01 กินพลังงานไฟสูงถึง 1.45 หน่วย/ตร.ม./วัน (เกินเกณฑ์ควบคุมประหยัดพลังงานที่ <=1.2 หน่วย)",
        anomalies: [
          "ห้องเย็นแช่แข็ง A-01 สิ้นเปลืองไฟพุ่งแตะ 1.45 หน่วยต่อตารางเมตรต่อวัน หลุดเป้าประหยัด",
          "คอมเพรสเซอร์เครื่องแช่แข็งรันโอเวอร์โหลดจากช่องประตูกันเย็นรั่วซึมสะสมความชื้น"
        ],
        recommendations: [
          "สั่งวิศวกรและช่างห้องเย็นเปลี่ยนแผงปะเก็นยางขอบประตูกันเย็นรั่วไหลห้อง A-01 ด่วนที่สุด",
          "ปรับการตั้งเวลาละลายเกล็ดน้ำแข็ง Defrost เป็นช่วงตี 1-4 เพื่อหลีกเลี่ยงรอบค่าไฟฟ้าพุ่ง"
        ]
      });
    } else if (departmentId === "d-clean") {
      return res.status(200).json({
